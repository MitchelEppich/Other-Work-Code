...
public class ParabolicPointer : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (ShouldDrawMarker)
            Graphics.DrawMesh(SelectionPadMesh, Matrix4x4.TRS(SelectedPoint + Vector3.up * 0.05f, Quaternion.LookRotation(new Vector3(CameraTransform.forward.x, 0, CameraTransform.forward.z), Vector3.up), Vector3.one * 0.2f), SelectionPadFadeMaterial, 0, null, 0);
    }
